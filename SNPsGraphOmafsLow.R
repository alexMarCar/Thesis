...
graphSNPmaker3 <- function(SNPdata,SNPs,m,a) {
  library(igraph)
  ...
  SelectedControls = order(OMAFcontrols, decreasing = F)[1:SNPs]
  SelectedCases = order(OMAFcases, decreasing = F)[1:SNPs]
  ...
  saveRDS(object = graphdf, file = paste0("~/TFM/SNPs/",SNPs,"GraphControlsLow.rds"))
  ...
  saveRDS(object = graphdf, file = paste0("~/TFM/SNPs/",SNPs,"GraphCasesLow.rds"))
