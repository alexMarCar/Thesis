...
graphSNPmaker <- function(SNPdata,m,a) {
  library(igraph)
  ...
  #Then,we can filter these datasets according to MOMAF (m) value, which is the threshold we use to get rid of non recurrent   SNPs.
  ...
  controlsDF2 = controlsDF[,rownames(controlsSNPSelect)]
  rownames(controlsDF2) = controlsDF$ID
  casesDF2 = casesDF[,rownames(casesSNPSelect)]
  ...
  saveRDS(object = graphdf, file = paste0(samplesControls,"GraphControls.rds"))
  ...
  saveRDS(object = graphdf, file = paste0(samplesCases,"GraphCases.rds"))
