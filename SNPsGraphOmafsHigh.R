...
graphSNPmaker2 <- function(SNPdata,SNPs,a) {
  library(igraph)
  ...
  SelectedControls = order(OMAFcontrols, decreasing = T)[1:SNPs]
  SelectedCases = order(OMAFcases, decreasing = T)[1:SNPs]
  ...
  controlsDF2 = controlsDF2[,SelectedControls]
  rownames(controlsDF2) = controlsDF$ID
  casesDF2 = casesDF2[,SelectedCases]
  ...
  saveRDS(object = graphdf, file = paste0("~/TFM/SNPs/",SNPs,"GraphControlsHigh.rds"))
  ...
  saveRDS(object = graphdf, file = paste0("~/TFM/SNPs/",SNPs,"GraphCasesHigh.rds"))
